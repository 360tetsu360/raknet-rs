...
impl Writer {
    ...
    pub fn write(&mut self,v : &[u8]) -> Result<()> { self.cursor.write(v)?;Ok(()) }

    pub fn write_u8(&mut self, v: u8) -> Result<()> { self.cursor.write_u8(v) }
    ...
    pub fn write_magic(&mut self) -> Result<usize> {
        let magic = [0x00,0xff,0xff,0x00,0xfe,0xfe,0xfe,0xfe,0xfd,0xfd,0xfd,0xfd,0x12,0x34,0x56,0x78];
