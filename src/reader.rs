use std::{
    io::{Cursor, Result, Read,ErrorKind},
    str,
    convert::TryInto,
...
impl<'a> Reader<'a> {
    ...
    pub fn read_u8(&mut self) -> Result<u8> { self.cursor.read_u8() }
    ...
    pub fn read_magic(&mut self) -> Result<[u8;16]> {
        let mut magic = [0;16];
        self.cursor.read(&mut magic).expect("Unable to read magic bytes");
        let offline_magic = [0x00,0xff,0xff,0x00,0xfe,0xfe,0xfe,0xfe,0xfd,0xfd,0xfd,0xfd,0x12,0x34,0x56,0x78];
        if magic == offline_magic {
            Ok(magic)
        }else{
            Err(std::io::Error::new(ErrorKind::Other,"Unable to read magic bytes"))
        }
    }
    pub fn read_address(&mut self) -> Result<SocketAddr> {
        ...
        if ip_ver == 4 {
            let ip = Ipv4Addr::new(0xff - self.read_u8()?,0xff - self.read_u8()?,0xff - self.read_u8()?,0xff - self.read_u8()?);
            ...
            self.cursor.read(&mut addr_buf).expect("Unable to read ipv6 address bytes");
            self.next(4);
            Ok(SocketAddr::new(IpAddr::V6(Ipv6Addr::new(
